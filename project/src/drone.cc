...
Drone::Drone(std::vector<float> position, std::vector<float> direction,
  ...
  if (JsonHelper::ContainsKey(details, "path")) {
     ...
     if (path_type == "smart") {
        ...
        path_strategy->setSpeed(speed_);
     }
     else if (path_type == "parabolic") {
        path_strategy = new Parabolic();
     }
     else {
...
void Drone::UpdateEntity(float dt) {
  ...
  //ONLY do the following if the drone is "dynamic" otherwise will throw exceptions
  //from the following vector operations
  //the drone does not have the package so it must get it
  if (has_packages_ == false && is_dynamic_ == true) { 
    ...
    //if the drone hasn't reached a node in the path then move towards it
    if (m->CalcMagnitude(direction_) > radius_) {
      ...
      //if the drones battery is dead it cannot move
      if (battery_->IsDead()) {
        is_dynamic_ = false;
    ...
    else {    //if the drone has reached a node then get the next node
      all_travel_paths_.at(0).erase(all_travel_paths_.at(0).begin());
    }

    //the drone has reached the package and must pick it up
  ...
  //now the drone has the package and must deliver to the customer
    ...
    //the drone has reached the customer and the package 
    ...
    if (all_delivery_paths_.at(0).size() == 0) {
      ...
      //the drone to the customer that was just reached.
